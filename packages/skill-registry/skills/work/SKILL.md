...
```
the implementation step worktree:<branch>(optional) specs:<path>(multiple,optional) <string(greedy)>(optional)
```
...
Copyable fallback:
```
the review step plan:<plan-path>
```
...
If `ask_user` is available, offer the same review handoff with `prefill: "the review step plan:<plan-path>"`. Copyable fallback:
```
the review step plan:<plan-path>
```
...
If user runs `the implementation step` and plan has `completed:` or `in-progress:` tasks:
1. Read plan
