...
```
the full pipeline <string(greedy)>(optional) plan:<path>(optional) specs:<path>(optional)
```
...
Suggest next steps:
- `the consolidate step` — save learnings
- `the issue scan` — deeper code review
- `the document step` — generate docs
...
> Resume with: `the full pipeline plan:<plan-path>`"
