...
```
the fix step debug:<path>(optional) <string(greedy)>(optional)
```
...
When user types `the fix step debug:`, autocomplete shows:
...
- Fixes without debug reports should use `a quick fix` instead
