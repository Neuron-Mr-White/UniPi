...
```
/unipi:plan [on|off|view|approve]
```

- no argument toggles plan mode; `on`/`off` are explicit, `view` shows the current
...
- [ ] Plan is focused enough for single `the implementation step` session
...
- **Worktree branch** → `the implementation step worktree:<branch-name> specs:YYYY-MM-DD-<topic>-plan`
- **Main branch** → `the implementation step specs:YYYY-MM-DD-<topic>-plan` (no worktree arg)

Copyable fallback commands when `ask_user` is unavailable:
```
the implementation step worktree:<branch-name> specs:YYYY-MM-DD-<topic>-plan
the implementation step specs:YYYY-MM-DD-<topic>-plan
the full pipeline plan:YYYY-MM-DD-<topic>-plan.md
```
...
If plan mode starts on an existing plan:
