...
- If all merged cleanly → suggest `the consolidate step` to save learnings
- If conflicts remain → suggest resolving and retrying `the implementation steptree-merge`
- If specs/plans exist for merged work → suggest `the document step` to update docs
