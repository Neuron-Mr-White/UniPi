...
**Note:** For active debugging of specific bugs, use `the debug step` instead. scan-issues finds potential problems; debug diagnoses known issues.

## Command Format

```
the issue scan <string(greedy)>(optional)
```
...
```
a quick work pass "fix critical security issue in auth.ts"
```

**If many issues:**
> "Multiple issues found. Consider planning a cleanup sprint."
```
the brainstorm skill "tech debt cleanup plan"
```
...
| Aspect | `the issue scan` | `the debug step` |
|--------|----------------------|----------------|
...
| Handoff | `a quick work pass` or `the brainstorm skill` | `the fix step` |
