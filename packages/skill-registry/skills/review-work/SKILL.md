---
name: review-work
description: "Review work — check task completion, run lint/build, mark reviewer remarks. Use after the implementation step completes."
---
...
```
the review step plan:<path>(optional) <string(greedy)>(optional)
```
...
Copyable fallback:
```
the implementation steptree-merge
```

*If `workbranch` is empty (main branch):*
> "All tasks complete and verified. Changes already on main — no merge needed."

Offer `the consolidate step` as a `new_session` handoff when available. Copyable fallback:
```
...
Offer the applicable `the implementation step ...` command as a `new_session` handoff when available.

*If `workbranch` is set:*
```
the implementation step worktree:<branch> specs:<plan-path>
```

*If `workbranch` is empty (main):*
```
the implementation step specs:<plan-path>
```
...
```
the review step plan:<plan-path>  (full review)
the implementation step specs:<plan-path>        (continue work)
```
...
- This is a checkpoint, not a fix pass — work continues via `the implementation step`
