...
import { ctxSearch, type CtxSearchInput, type ThrottledSearchResult } from "./ctx-search.js";
...
const CtxSearchParams = Type.Object({
  ...
  mode: Type.Optional(Type.Union(
    [Type.Literal("porter"), Type.Literal("trigram"), Type.Literal("rrf"), Type.Literal("fuzzy")],
    { description: "Search mode (default: rrf)" },
  )),
  sort: Type.Optional(Type.Union(
    [Type.Literal("relevance"), Type.Literal("timeline")],
    { description: "Sort mode: relevance (default) or timeline (chronological across sources)" },
  )),
  projectDir: Type.Optional(Type.String({ description: "Project directory for timeline search" })),
});
...
const CtxFetchAndIndexParams = Type.Object({
  ...
  chunkSize: Type.Optional(Type.Number({ description: "Chunk size in characters", minimum: 100 })),
  force: Type.Optional(Type.Boolean({ description: "Bypass 24h TTL cache and re-fetch" })),
  contentType: Type.Optional(Type.Union(
    [Type.Literal("markdown"), Type.Literal("json"), Type.Literal("plain")],
    { description: "Content type for chunking strategy" },
  )),
});
...
export function registerCompactorTools(pi: ExtensionAPI, deps: CompactorToolDeps): void {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params: Static<typeof CtxSearchParams>): Promise<any> {
      try {
        const result = await ctxSearch(params as CtxSearchInput);

        // Handle blocked state
        if (result.blocked) {
          return textResult(result.warning!, { blocked: true, callCount: result.callCount });
        }

        const results = result.results;
        if (results.length === 0) {
          return textResult(`No results for "${params.query}".${result.warning ? `\n\n${result.warning}` : ""}`);
        }
        ...
        return textResult(
          ...
          { results, throttled: result.throttled, callCount: result.callCount } as unknown as Record<string, unknown>,
        );
      } catch (err) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params: Static<typeof CtxFetchAndIndexParams>): Promise<any> {
      try {
        ...
        if (result.cacheHit) {
          return textResult(
            `Cached: "${result.label}" — ${result.totalChunks} sections, indexed at ${result.cachedAt} (fresh, TTL: 24h). To refresh: call ctx_fetch_and_index with force: true. Use ctx_search to query content.`,
            result as unknown as Record<string, unknown>,
          );
        }
        return textResult(
