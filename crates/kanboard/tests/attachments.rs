...
use common::{cli, http, http_bytes, Daemon, Fixture};
...
fn header(headers: &[(String, String)], name: &str) -> String {
    headers.iter().find(|(key, _)| key == name).map(|(_, value)| value.clone()).unwrap_or_default()
}
...
fn cli_attach_stores_the_file_and_logs_a_comment_that_embeds_it() {
    ...
    let out = cli(&fixture, &["attach", &task.id, file.to_str().unwrap(), "--note", "the broken header", "--json"]);
    ...
    assert!(reference.starts_with(&format!("att:{}/", task.id)), "{reference}");
    assert_eq!(attachment["markdown"], format!("![Screen-Shot-1.png]({reference})"));
    assert_eq!(std::fs::read(attachment["path"].as_str().unwrap()).unwrap(), PNG);

    // The comment carries the note and the embed, and survives the file round-trip.
    let last = payload["activity"].as_array().unwrap().last().unwrap()["text"].as_str().unwrap().to_string();
    assert_eq!(last, format!("the broken header\n![Screen-Shot-1.png]({reference})"));
    let reread = fixture.tasks().into_iter().find(|t| t.id == task.id).unwrap();
    ...
    let again = cli(&fixture, &["attach", &task.id, file.to_str().unwrap(), "--json"]);
    ...
    assert_eq!(items.as_array().unwrap().len(), 1, "same content is stored once");
...
fn http_upload_then_serve_with_safe_headers() {
    ...
    let (status, _, body) = http_bytes(
        daemon.port,
        "POST",
        &format!("/api/tasks/{slug}/{}/attachments?name=crash%20log.txt", task.id),
        b"panic at line 42\n",
        &[],
    )
    .unwrap();
    ...
    assert_eq!(uploaded["markdown"], format!("[crash-log.txt]({})", uploaded["ref"].as_str().unwrap()));

    // Upload alone logs nothing — the UI posts the comment separately.
    let shown = http(daemon.port, "GET", &format!("/api/tasks/{slug}/{}", task.id), None).unwrap().json();
    assert!(!shown["activity"].as_array().unwrap().iter().any(|entry| entry["text"].as_str().unwrap_or("").contains("att:")));
    assert_eq!(shown["attachments"].as_array().unwrap().len(), 1);

    let name = uploaded["name"].as_str().unwrap();
    let (status, headers, bytes) =
        http_bytes(daemon.port, "GET", &format!("/api/files/{slug}/{}/{name}", task.id), b"", &[]).unwrap();
    assert_eq!(status, 200);
    assert_eq!(bytes, b"panic at line 42\n");
    assert_eq!(header(&headers, "content-type"), "text/plain; charset=utf-8");
    ...
    let (_, headers, _) = http_bytes(
        daemon.port,
        "GET",
        &format!("/api/files/{slug}/{}/{}", task.id, evil["name"].as_str().unwrap()),
        b"",
        &[],
    )
    .unwrap();
...
fn traversal_unknown_task_and_oversize_are_refused() {
    ...
    let (status, _, _) =
        http_bytes(daemon.port, "POST", &format!("/api/tasks/{slug}/NOPE-1/attachments?name=a.txt"), b"x", &[]).unwrap();
    assert_eq!(status, 404);

    let big = vec![0u8; kanboard::attachments::MAX_BYTES + 4096];
    let (status, _, _) =
        http_bytes(daemon.port, "POST", &format!("/api/tasks/{slug}/{}/attachments?name=big.bin", task.id), &big, &[]).unwrap();
    assert!(status == 413 || status == 0, "oversized upload refused (got {status})");
...
fn remote_bind_requires_the_token_for_files() {
    ...
    std::fs::write(&file, PNG).unwrap();
    let out = cli(&fixture, &["attach", &task.id, file.to_str().unwrap(), "--json"]);
    let payload: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    ...
    let (status, _, bytes) = http_bytes(daemon.port, "GET", &path, b"", &[("authorization", &auth)]).unwrap();
    assert_eq!(status, 200);
    assert_eq!(bytes, PNG);
}
