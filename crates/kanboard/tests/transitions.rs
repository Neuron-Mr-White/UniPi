...
use kanboard::model::{Actor, Priority, Staleness, Status};
use kanboard::transitions::{self, RULES, comment_required, rule_for};
...
fn every_table_row_is_reachable_and_gated() {
    ...
    for rule in RULES {
        ...
        let comment = if comment_required(rule.from, rule.to) {
            REASON
        } else {
            None
        };
        ...
        if comment_required(rule.from, rule.to) {
            let err =
                transitions::check(rule.from, rule.to, actor, None, Staleness::Stale).unwrap_err();
...
fn cancelling_is_user_only_and_only_from_the_open_lanes() {
    ...
    .unwrap_err();
    assert!(
        err.to_string().contains("rework note"),
        "hint the path back: {err}"
    );
}
