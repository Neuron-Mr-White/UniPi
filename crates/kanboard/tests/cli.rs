...
fn add_emits_a_task_and_json_matches_the_file() {
    ...
    let run = kb(&fixture, &["add", "Write the README", "--json", "--priority", "high"]);
...
fn add_rejects_a_lane_it_may_not_target() {
    let fixture = Fixture::new();
    let run = kb(&fixture, &["add", "nope", "--status", "in_review", "--json"]);
...
fn move_denied_reports_the_rule_and_exits_one() {
    let fixture = Fixture::new();
    let task = fixture.add_with("t", kanboard::model::Status::Todo, kanboard::model::Priority::None, &[]);
    fixture.claim_next("s", std::process::id());
    // in_progress → in_review as a user needs a comment AND the system actor.
    let run = kb(&fixture, &["move", &task.id, "in_review", "--json"]);
    assert_eq!(run.code, 1);
    assert!(run.json["error"].as_str().unwrap().contains("system only"), "{run:?}");
    ...
    let run = kb(&fixture, &["move", &task.id, "todo", "--comment", "needs tests", "--json"]);
...
fn move_to_a_final_lane_is_refused_with_a_hint() {
    ...
    assert!(run.json["error"].as_str().unwrap().contains("only done/cancelled"), "{}", run.stderr);
}
...
fn claim_next_json_reports_waiting_reasons_and_chains_exactly_once() {
    ...
    let run = kb(
        &fixture,
        &["claim-next", "--session", "s1", "--pid", "999999", "--host", "test-host", "--json"],
    );
    ...
    let run = kb(
        &fixture,
        &["claim-next", "--session", "s2", "--pid", "999998", "--host", "test-host", "--json"],
    );
    assert!(run.json["task"].is_null(), "the dependent task is not ready");
...
fn validate_reports_line_numbers_and_fails_the_exit_code() {
    ...
    assert!(problem["message"].as_str().unwrap().contains("unknown status \"nonsense\""));
...
fn project_lifecycle_over_the_cli() {
    ...
    let run = kb(&fixture, &["project", "add", "--name", "Other", "--prefix", "oth", "--json"]);
...
fn show_includes_deps_status_and_staleness() {
    ...
    assert_eq!(run.json["staleness"], "running", "no run block means not stale");
...
fn note_edit_link_order_and_duplicate_round_trip() {
    ...
    assert_eq!(run.json["activity"].as_array().unwrap().last().unwrap()["text"], "remember this");

    let run = kb(&fixture, &["edit", &a.id, "--title", "renamed", "--priority", "urgent", "--json"]);
...
fn archive_sweep_reports_the_days_threshold() {
    ...
    assert_eq!(run.json["skipped"], "archiveAfterDays is 0 (off)");
...
fn actor_env_is_honoured_and_reported_in_activity() {
    ...
    assert_eq!(stored.activity.last().unwrap().actor, kanboard::model::Actor::Agent);
}
...
#[test]
fn eight_processes_claiming_three_tasks_produce_exactly_three_claims() {
    let fixture = Fixture::new();
    for title in ["one", "two", "three"] {
        fixture.add_with(title, kanboard::model::Status::Todo, kanboard::model::Priority::None, &[]);
    }
    ...
    for child in children {
        ...
        assert!(output.status.success(), "claim-next failed: {output:?}");
    ...
    assert_eq!(claimed.len(), 3, "exactly three claims: {claimed:?}");
