...
- **kanboard: remote access with a token gate.** `unipi-kanboard serve --host <addr>` (default `127.0.0.1`); any non-loopback bind generates a 32-byte base64url token, stores `host`/`token` in `daemon.json`, and requires it on every request as `?t=<token>` (sets an `HttpOnly; SameSite=Strict` cookie and 303-redirects to the URL without it), the `kb_token` cookie, or `Authorization: Bearer <token>` — compared in constant time, 401 otherwise. `/api/health` stays open but hides the pid off-loopback, POSTs whose `Origin` disagrees with `Host` are refused in both modes, and loopback binds stay token-free. `/unipi:kanboard open [--host H] [--port N]` (completions: `127.0.0.1`, `0.0.0.0`, `tailscale` → `tailscale ip -4`) overrides the new `host`/`port` settings per invocation, prints every reachable URL with the token (hostname + non-internal IPv4s + tailnet address for a wildcard bind), the `ssh -N -L …` tunnel hint for loopback, and a loud "reachable from the network" warning; a daemon running on another binding is stopped and restarted (`restarted kanboard on 0.0.0.0:37473`).

### Changed

- **kanboard: platform binaries and packaging.** Five `@pi-unipi/kanboard-<platform>` packages (`linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, `win32-x64`) hang off `@pi-unipi/kanboard` as `optionalDependencies`, each with `os`/`cpu` gates so npm installs exactly one. `.github/workflows/kanboard-binaries.yml` builds them per target (Linux via musl → static-pie binaries, ~3.9 MB), runs tests + clippy on native targets, uploads one artifact per platform and, on a `v*` tag, attaches them to the GitHub release; npm publishing happens only when an `NPM_TOKEN` secret exists (there is none today), and `npm run publish:kanboard` **skips any platform package whose `bin/` is empty** with a loud warning and a non-zero exit. `ci.yml` now also runs `cargo test` + `cargo clippy -D warnings` for `crates/kanboard`. A local, offline packaging proof lives in `scripts/test-kanboard-packaging.mjs` (pack both packages, lay them out as `node_modules`, resolve through `src/bin.ts`, run `--version`).
- **kanboard: one unparseable task file no longer takes the board offline.** The store loads tolerantly — a file that fails validation is skipped (never partially trusted) and reported as `problems: [{file, line, error}]` by `list --json` and the API (`{tasks, problems}`); writing to a broken file answers `UNI-5 is unreadable: <message> (line N); run unipi-kanboard validate --fix`; the board page shows a red "N task file(s) need repair" banner and still renders everything else; `claim-next` ignores broken files.
- **workflow: the agent's board CLI no longer prompts in auto mode.** A simple command whose program is `unipi-kanboard` and whose args carry `--actor agent` classifies as *kanboard CLI* and runs in `auto`/`full`; `ask` mode still asks, `--actor user|system` is never covered, compound commands keep the per-part rules, and plan mode does not inherit the allowance.

### Changed (earlier)

- **`@pi-unipi/kanboard` is now the v3 terminal client** for the Rust board: `/unipi:kanboard [open|onboard|add|work|stop|status]` with arg completions. `open` ensures the daemon (reuse a healthy one, else spawn `serve` detached) and prints the board URL; `onboard` is idempotent and reveals the skill; `add <text>` is quick capture into Backlog with no agent turn; `status` reports daemon, lane counts and the runner. Binary resolution: `UNIPI_KANBOARD_BIN` → `@pi-unipi/kanboard-<platform>-<arch>` → the dev build; nothing found means every command reports `kanboard binary unavailable for <platform>-<arch>` and does nothing else.
- **kanboard: the task runner (`work`).** One job per session, with the runner owning the transitions agents may not write: `claim-next` → jev picks the mode (`direct`/`plan`/`goal`, null → direct, logged to `~/.unipi/logs/kanboard.log` under `UNIPI_DEBUG_KANBOARD=1`) → `set-run` → the task (title, body, last 10 activity entries, deps with status and last note, and the rules) goes to the agent as a user message. Run end releases to In Review with the last assistant text (≤500 chars); a task the agent blocked is respected and the loop continues; Esc releases to Todo with "interrupted by user" and stops; session shutdown releases to Todo. `plan` mode drives workflow's plan mode through the new `unipi:plan-enter`/`unipi:plan-exit` command runners (a discarded plan releases to Todo); `goal` mode starts a long-horizon goal through `unipi:goal-start`, records the goal id with `set-run --goal`, and reads completion from `unipi:goal-status`. Footer shows `▣ UNI-12 · direct`; the claim is persisted with `appendEntry` so a reload offers resume or release.
- **kanboard: `skills/kanboard/SKILL.md`** teaches the CLI and the rules (never `--actor user`, never move to in_review/done, block with a comment to ask the user, file follow-ups in Backlog). `onboard`/`work` force-reveal it through the new `unipi:skills:reveal` event, which utility turns into the usual append-only reveal message — the system prompt stays untouched. Hub section "Kanboard" (namespace `kanboard`): `chainGate`, `continue`, `idleMin`, `port`, `archiveAfterDays` (sweep on session start), `openBrowser`, plus "Open board…" and "Stop daemon" actions.
- **core: `callCommandRunner`** — call another module's registered runner and read its result (`registerCommandRunner` handlers may now take an `args` payload and return a value). Used by kanboard for plan/goal, and by the settings hub as before.
...
- **kanboard: the old htmx/Alpine board is gone** — `parser/`, `server/`, `ui/`, `commands.ts`, `types.ts` and the `kanboard-doctor` skill (with its autocomplete and core constants). The v3 board is the Rust binary plus the Topcoat UI inside it.
