...
/unipi:brainstorm <topic>
    └── generates: docs/specs/YYYY-MM-DD-<topic>-design.md
        └── prefix: specs:

/unipi:plan specs:<file> (multiple allowed)
    └── generates: docs/plans/YYYY-MM-DD-<topic>-plan.md
        └── prefix: plan:
...
```
Quick:     /unipi:quick-work <task>              → docs/quick-work/
Debug:     /unipi:debug <bug> → /unipi:fix       → docs/debug/ + docs/fix/
Research:  /unipi:research <topic>                → docs/research/
Context:   /unipi:gather-context <topic>          → docs/context-gathered/
Scan:      /unipi:scan-issues <scope>             → findings in conversation
Document:  /unipi:document <target>               → docs/generated/
Chore:     /unipi:chore-create → chore-execute    → docs/chore/
